...
const open = computed(() => !!props.typeDetail);
...
function closeDrawer() {
    ...
    delete next.type;
    router.get(route('policy-types.index'), next, {
        replace: true,
        preserveScroll: true,
        preserveState: true,
        only: ['typeDetail'],
...
const IconC = computed(() => policyTypeOutlineIcon(props.typeDetail?.icon || 'rectangle-stack'));
...
function submitAdd() {
    ...
    addForm
        .transform((data) => ({
            policy_type_id: props.typeDetail.id,
    ...
    <Teleport to="body">
        ...
        >
            ...
            >
                ...
                >
                    <aside
                        v-if="open && typeDetail"
                        class="flex h-full w-full max-w-2xl flex-col border-l border-slate-200/90 bg-white shadow-2xl dark:border-slate-800 dark:bg-slate-950"
                        @click.stop
                    >
                        <header
                        ...
                        >
                            <div class="flex min-w-0 flex-1 items-center gap-3">
                                <div
                                    class="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl ring-2 ring-white shadow-md dark:ring-slate-900"
                                    :style="{
                                        backgroundColor: (typeDetail.color || '#6366f1') + '33',
                                        color: typeDetail.color || '#6366f1',
                                    }"
                                >
                                    <component :is="IconC" class="h-6 w-6" />
                                </div>
                                <div class="min-w-0">
                                    <h2 class="text-xl font-bold tracking-tight text-slate-900 dark:text-white">
                                        {{ typeDetail.name }}
                                    </h2>
                                    <p class="font-mono text-sm text-slate-500">{{ typeDetail.slug }}</p>
                                </div>
                            </div>
                            <button
                                type="button"
                        ...
                        <div class="flex-1 overflow-y-auto px-6 py-6">
                            ...
                            >
                                <p class="text-xs font-semibold uppercase tracking-wide text-slate-400">Genel</p>
                                <p
                                    v-if="typeDetail.description"
                                    class="mt-2 text-sm leading-relaxed text-slate-700 dark:text-slate-300"
                                >
                                    {{ typeDetail.description }}
                                </p>
                                <p v-else class="mt-2 text-sm text-slate-500">Açıklama yok.</p>
                                <dl class="mt-4 grid gap-2 text-sm sm:grid-cols-2">
                                    <div>
                                        <dt class="text-slate-500">Kategori</dt>
                                        <dd class="font-medium">{{ typeDetail.category || '—' }}</dd>
                                    </div>
                                    <div>
                                        <dt class="text-slate-500">Renk</dt>
                                        <dd class="flex items-center gap-2">
                                            <span
                                                class="inline-block h-5 w-5 rounded-md ring-1 ring-slate-200 dark:ring-slate-600"
                                                :style="{ backgroundColor: typeDetail.color }"
                                            />
                                            <span class="font-mono text-xs">{{ typeDetail.color }}</span>
                                        </dd>
                                    </div>
                                    <div>
                                        <dt class="text-slate-500">İkon</dt>
                                        <dd class="font-mono text-xs">{{ typeDetail.icon }}</dd>
                                    </div>
                                    <div>
                                ...
                                <div class="flex items-start justify-between gap-3">
                                    <div>
                                        <h3 class="text-sm font-bold text-slate-900 dark:text-white">
                                            Bu tür hangi firmalarda?
                                        </h3>
                                        <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">
                                            Komisyon oranı firma + tür çiftine göre tanımlanır.
                                        </p>
                                    </div>
                                    <button
                                        v-if="(typeDetail.attachable_companies || []).length"
                                        type="button"
                                        class="shrink-0 rounded-xl bg-violet-600 px-3 py-1.5 text-xs font-semibold text-white shadow-sm transition hover:bg-violet-500"
                                        @click="openAdd"
                                    >
                                        Yeni firma
                                    </button>
                                        ...
                                        <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                                            ...
                                            >
                                                <td class="px-3 py-2.5">
                                                    ...
                                                    <div v-if="row.code" class="font-mono text-xs text-slate-500">
                                                        {{ row.code }}
                                                    </div>
                                                </td>
                                                <td class="px-3 py-2.5">
                                                ...
                                                <td class="px-3 py-2.5 text-right">
                                                    <div class="flex justify-end gap-1">
                                                        <template v-if="editingId === row.id">
                                                            <button
                                                                type="button"
                                                                class="rounded-lg px-2 py-1 text-xs font-semibold text-violet-600 hover:bg-violet-50 dark:hover:bg-violet-950/40"
                                                                @click="saveRow(row)"
                                                            >
                                                                Kaydet
                                                            </button>
                            ...
                            <section class="mt-8">
                                ...
                                <p class="mt-2 text-sm text-slate-600 dark:text-slate-400">
                                    ...
                                    <span class="font-semibold text-slate-900 dark:text-white">
                                        {{ typeDetail.renewal_reminder_days }} gün
                                    </span>
                                </p>
                            </section>

                            <section class="mt-8">
                                <h3 class="text-sm font-bold text-slate-900 dark:text-white">İstatistik</h3>
                                <div class="mt-3 grid grid-cols-2 gap-3">
                                    <div
                                        class="rounded-xl border border-slate-100 p-3 dark:border-slate-800 dark:bg-slate-900/40"
                                    >
                                        <p class="text-xs text-slate-500">Poliçe</p>
                                        <p class="text-lg font-bold">{{ typeDetail.stats.policies_count }}</p>
                                    </div>
                                    <div
                                        class="rounded-xl border border-slate-100 p-3 dark:border-slate-800 dark:bg-slate-900/40"
                                    >
                                        ...
                                        <p class="text-sm font-bold tabular-nums">
                                            {{ formatMoneyTr(typeDetail.stats.premium_volume) }}
                                        </p>
                                    </div>
                                </div>
                            </section>
                            ...
                            <Link
                                :href="route('policy-types.edit', typeDetail.id)"
                                class="mt-8 inline-flex w-full items-center justify-center rounded-xl bg-indigo-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-indigo-500"
                            >
                                Tam düzenleme
                            </Link>
                        </div>
        ...
        >
            <div
                ...
                @click.stop
            >
                <h3 class="text-lg font-bold text-slate-900 dark:text-white">Sigorta firması ekle</h3>
                <form class="mt-4 space-y-4" @submit.prevent="submitAdd">
                    <div>
                        <label class="text-sm font-medium text-slate-700 dark:text-slate-300">Firma</label>
                        <select
                        ...
                        >
                            ...
                            <option v-for="c in typeDetail.attachable_companies" :key="c.id" :value="c.id">
                                {{ c.name }} {{ c.code ? `(${c.code})` : '' }}
                            </option>
                        </select>
                        <InputError class="mt-1" :message="addForm.errors.insurance_company_id" />
                    </div>
                    <div>
